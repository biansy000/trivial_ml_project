...
def train_net(net,
    ...
    for epoch in range(epochs):
        ...
        with tqdm(total=25, desc=f'Epoch {epoch + 1}/{epochs}', unit='img') as pbar:
            for imgs, labels in train_loader:
                ...
                masks_pred = net(imgs)
                # print(masks_pred.shape, labels.shape)
                target_size = (labels.shape[1], labels.shape[2])
                masks_pred = F.interpolate(masks_pred, size=target_size, mode='bilinear', align_corners=True)
                # target_size = (masks_pred.shape[-2], masks_pred.shape[-1])
                true_masks = true_masks.unsqueeze(1)
                # true_masks = F.interpolate(true_masks, size=target_size, mode='bilinear', align_corners=True)
    ...
    with open('train_hrnet_res.txt', 'w') as f:
        write_lists = \
