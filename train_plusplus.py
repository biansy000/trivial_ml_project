...
def train_net(net,
    ...
    # optimizer = optim.SGD(net.parameters(), lr=lr*100, momentum=0.9, nesterov=False, weight_decay=1e-4)
    ...
    for epoch in range(epochs):
        ...
        with tqdm(total=25, desc=f'Epoch {epoch + 1}/{epochs}', unit='img') as pbar:
            for imgs, labels in train_loader:
                ...
                masks_pred = net(imgs)
                masks_pred = torch.stack(masks_pred, dim=0).squeeze(2)
                # for item in masks_pred:
                #     print(item.shape)
                ...
                iou, size = calc_IoU(masks_pred[-1], true_masks.squeeze(0))
                IoU.update(iou, size)
                acc, size = calc_Acc(masks_pred[-1], true_masks.squeeze(0))
                Acc.update(acc, size)
        ...
        tmp_iou, tmp_vinfo, tmp_vrand = valid(net, device, val_loader, deep_supervision)
    ...
    with open('train_plus_plus_res.txt', 'w') as f:
        write_lists = \
...
def valid(net, device, loader, deep_supervision):
    ...
    with tqdm(total=n_val, desc='Validation round', unit='batch', leave=False) as pbar:
        for i, (imgs, labels) in enumerate(loader):
            ...
            with torch.no_grad():
                ...
                true_masks = torch.cat([true_masks for _ in range(super_dim)], dim=0)
                true_masks = F.interpolate(true_masks, size=target_size, mode='nearest')
            
            draw_label = mask_pred[-1, 0].detach().cpu().numpy()
            ...
            iou, s = calc_IoU(mask_pred[-1], true_masks[-1].squeeze(0))
            IoU.update(iou, s)
            acc, s = calc_Acc(mask_pred[-1], true_masks[-1].squeeze(0))
            Acc.update(acc, s)
...
def get_args():
    ...
    parser.add_argument('-ds', '--deep_supervision', dest='deep_supervision', type=bool, default=True)
...
if __name__ == '__main__':
    ...
    net = NestedUNet(num_classes=1, deep_supervision=args.deep_supervision)
    ...
    logging.info(f'Network:\n'
                 f'\tUse deep Supervision {net.deep_supervision}\n')
